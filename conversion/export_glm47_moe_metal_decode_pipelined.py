"""Export a DECODE-ONLY GLM-4.7-Flash bundle whose 64-expert top-4 MoE runs the ``gather_qmm``
custom Metal kernel — reading ONLY the 4 routed experts (a 16× over-read removed).

The metal-MoE sibling of ``export_glm47_decode_pipelined.py``. Everything outside the routed
experts is identical to the shipped int8 recipe (int8 per-block-32 linears incl. MLA + the shared
expert, fp16 router, absmax int8 head). The ONLY change is the routed-expert FFN: ``MetalSwitchGLU``
with the CLEAN ``sym8`` scheme (symmetric-linear int8 per-K-block-32 = the shipped int8lin recipe,
read via a bit-exact gather). GLM-4.7-Flash is MLA + 64e top-4 + non-gated shared expert, KV-only
stateful (no GDN/conv) — the gather composes the same way it does on LFM / Qwen3.6.

Run:  cd ~/code/coreai/coreai-models && .venv/bin/python \
          ../coreai-models-community/conversion/export_glm47_moe_metal_decode_pipelined.py \
          sym8 --hf-id zai-org/GLM-4.7-Flash
"""
...
def linear_quant_config(dtype: str = "int8") -> dict:
    """int8 per-block-32 for NON-routed-expert weights (MLA, shared expert). Routed experts
    (SwitchLinear) -> None (kept fp16) because the gather_qmm kernel owns them."""
...
def main() -> None:
    ...
    print(f"model ready | {cfg.num_hidden_layers} layers, E={cfg.n_routed_experts}/"
          f"top{cfg.num_experts_per_tok}, moe_inter={cfg.moe_intermediate_size}, "
    ...
    # 2) metalize the routed MoE: SwitchGLU -> MetalSwitchGLU (gather_qmm sym8; reads only 4/64)
    print(f"metalizing routed MoE -> gather_qmm sym8 (reads only top-{cfg.num_experts_per_tok}/"
          f"{cfg.n_routed_experts}) ...", flush=True)
    kernel = metalize_moe(model, scheme="sym8")

    # 3) export with the kernel; drop gather_mm (gone) + gated_delta_update (as shipped)
